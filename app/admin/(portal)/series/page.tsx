  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-[#021523]">Series</h1>
          <p className="text-sm text-[#818ea0]">{series?.length ?? 0} series</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-[#e5e8ec] overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#e5e8ec] bg-[#f7f8fa]">
              ...
              <th className="text-left px-4 py-3 font-semibold text-[#818ea0] text-xs uppercase tracking-wide">Brand</th>
          ...
          <tbody className="divide-y divide-[#f2f3f5]">
            {(series ?? []).map((s: any) => (
              <tr key={s.id} className="hover:bg-[#f7f8fa] transition-colors">
                ...
                <td className="px-4 py-3 text-[#818ea0] text-xs">{s.brand?.name ?? '—'}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <DeleteButton action={deleteSeries.bind(null, s.id)} label={s.name} />
                  </div>
                </td>
              </tr>
            ))}
        ...
        {(series ?? []).length === 0 && (
          <div className="text-center py-12 text-[#818ea0] text-sm">No series yet</div>
        )}
      </div>
    </div>
  )
