...
export default async function EditSeriesPage({ params }: Props) {
  ...
  const { data: s, error } = await supabaseAdmin.from('series').select('*').eq('id', parseInt(id)).maybeSingle()
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="text-xs text-[#818ea0] mb-1">
          <a href="/admin/series" className="hover:text-[#041e42]">Series</a> / Edit
        </div>
        <h1 className="text-2xl font-black text-[#021523]">Edit Series</h1>
        <p className="text-sm text-[#818ea0] mt-0.5">{s.name}</p>
      </div>
      <div className="bg-white rounded-xl border border-[#e5e8ec] p-6 max-w-lg">
        <EditSeriesForm series={s} />
      </div>
    </div>
  )
}

function EditSeriesForm({ series }: { series: any }) {
  const [state, action, pending] = useActionState(updateSeries, null)
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="name" defaultValue={series.name} required className={inputCls} />
      </div>
      <div>
        <label className={labelCls}>Slug *</label>
        <input type="text" name="slug" defaultValue={series.slug} required className={inputCls} />
      </div>
      <div>
        <label className={labelCls}>Brand ID *</label>
        <input type="number" name="brand_id" defaultValue={series.brand_id} required className={inputCls} />
      </div>
