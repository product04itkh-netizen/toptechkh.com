...
interface Props {
  ...
  categoryId: number
  brands: Brand[]
  existingBrandBanners: Record<number, string>
}

export default function CategoryEditForm({ category, action, categoryId, brands, existingBrandBanners }: Props) {
  const [state, formAction, pending] = useActionState(action, null as { error?: string } | null)
  const existingBanner = category.image_url?.includes('supabase') ? [category.image_url] : []
      ...
      <div className="bg-white rounded-xl border border-[#e5e8ec] p-6 max-w-2xl">
        <form action={formAction} className="space-y-5">
          ...
          <div>
            ...
            <ImageUpload name="banner_url" defaultImages={existingBanner} />
          </div>

          {/* Per-brand banners */}
          {brands.length > 0 && (
            <div>
              <div className="border-t border-[#e5e8ec] pt-5 mb-4">
                <h3 className="text-sm font-bold text-[#021523]">Brand Banners</h3>
                <p className="text-xs text-[#818ea0] mt-0.5">
                  Upload a banner for each brand — specific to <strong>{category.name}</strong> only
                </p>
              </div>
          ...
          <div className="flex gap-3 pt-2 border-t border-[#e5e8ec]">
            <button
              type="submit"
              disabled={pending}
